       ...
       // private NCacheSessionStateSettings _regionalCacheSessionStateSettings;
        ...
        public virtual void Initialize(string name, NameValueCollection config)
        {
            ...
            */
            ...
            //if (!string.IsNullOrEmpty(config["writeExceptionsToEventLog"]) && config["writeExceptionsToEventLog"].ToLower() == "true")
            //    _writeException = true;

            //#if !EXPRESS
            //            if (!string.IsNullOrEmpty(config["asyncSession"]) && config["asyncSession"].ToLower() == "true")
            //                _asyncOps = true;
            //#endif

            //if(!string.IsNullOrEmpty(config["enableLogs"]) && config["enableLogs"].ToLower() == "true")
            //    _logs = true;

            //if (_logs && !string.IsNullOrEmpty(config["enableDetailLogs"]) && config["enableDetailLogs"].ToLower() == "true")
            //    _detailedLogs = true;

            //if (!string.IsNullOrEmpty(config["enableSessionLocking"]) && config["enableSessionLocking"].ToLower() == "true")
            //    _lockSessions = true;           

            //get cache mode to initialize cache.
            //if (string.IsNullOrEmpty(config["useInProc"])) throw new ConfigurationException("useInProc");
            //if (config["useInProc"].ToLower() == "true") _inproc = true;

            //[Arif]: Removed after introduction of Multi-cache feature
            //if (!string.IsNullOrEmpty(config["group"])) _group = config["group"];
        ...
        //        [Ata]: This can only be specified in web.config. we will not keep record of appIds
        //        private void SetApplicationId()
        //        {
        //            if (_cache == null) return;

        //            if (s_applicationId == null || s_applicationId == string.Empty)
        //            {
        //                try
        //                {
        //                    this.AcquireAppLockOnCache();

        //                    object applicationId = _cache.Get(HttpRuntime.AppDomainAppId);

        //                    if (applicationId == null)
        //                    {
        //                        object cuurentId = _cache.Get(APP_ID);
        //                        s_applicationId = (cuurentId == null ? 1 : (int)cuurentId + 1).ToString();

        //#if !EXPRESS
        //                        _cache.Insert(APP_ID, s_applicationId);
        //                        _cache.Insert(HttpRuntime.AppDomainAppId, s_applicationId);
        //#else
        //                        _cache.Insert(APPID, s_applicationId, null, Cache.NoAbsoluteExpiration, Cache.NoSlidingExpiration, CacheItemPriority.Default);
        //                        _cache.Insert(HttpRuntime.AppDomainAppId, s_applicationId, null, Cache.NoAbsoluteExpiration, Cache.NoSlidingExpiration, CacheItemPriority.Default);
        //#endif
        //                    }
        //                    else
        //                    {
        //                        s_applicationId = (string)applicationId;
        //                    }
        //                }
        //                catch (Exception)
        //                {
        ...
        //                finally
        //                {
        //                    this.ReleaseAppLockOnCache();
        //                }
        //            }
        //        }
        //        [Ata]: This function may have issues with multiple threads. Review before use
        //        private void AcquireAppLockOnCache()
        //        {
        //            while (true)
        //            {
        //#if !EXPRESS
        //                if (_cache.Contains(APP_LOCK_KEY))
        //#else
        //                if(_cache.Get(APPLOCKKEY) == null)
        //#endif
        //                    Thread.Sleep(500);
        //                else
        //                {
        //#if !EXPRESS
        //                    _cache.Add(APP_LOCK_KEY, string.Empty);
        //#else
        //                    _cache.Add(APPLOCKKEY, string.Empty, null, Cache.NoAbsoluteExpiration, Cache.NoSlidingExpiration, CacheItemPriority.Default);
        //#endif
        //                    break;
        //                }
        //            }
        ...
        //private void ReleaseAppLockOnCache()
        //{
        //    _cache.Remove(APP_LOCK_KEY);
        //}
