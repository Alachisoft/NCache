...
#region PUB_SUB
           ...
           // if (SocketServer.IsServerCounterEnabled) _perfStatsCollector.MsecPerCacheOperationBeginSample();
            ...
            try
            {
                ...
                //if (LicenseManager.NCacheBuildType == LicenseManager.BuildType.ENTO)
                //{
                //    Exception error;
                //    if (!doThrottleCommand || CheckContinuationAndThrottle(clientManager, out error))
                //    {
                //        incommingCmd.ExecuteCommand(clientManager, command);
                //    }
                //    else
                //    {
                //        var response = ResponseHelper.SerializeExceptionResponseWithType(error, command.requestID, command.commandID);
                //        incommingCmd.SerializedResponsePackets.Add(response);
                //    }
                //}
                //else
                {
            ...
            //if (type == Common.Protobuf.Command.Type.INSERT)
            //{
            //    clientManager.ConnectionManager.ProccessCommand(cmd, clientManager, waitforRequests);
            //}
            ...
            try
            {
                if (clientManager != null && !clientManager.IsCacheStopped)
                {
                    if (incommingCmd.SerializedResponsePackets != null && !incommingCmd.IsCancelled && incommingCmd.SerializedResponsePackets.Count > 0)
                    {
                        ...
                        //{
                        //    foreach (var responseBytes in reponse)
                        //    {
                        //        clientManager.ConnectionManager.SendResponse((byte[])responseBytes, clientManager, waitforRequests);
                        //    }
                    ...
                    //if (!isAsync && command.type != Common.Protobuf.Command.Type.PING && (incommingCmd.SerializedResponsePackets == null || incommingCmd.SerializedResponsePackets.Count <= 0) && !incommingCmd.IsCancelled)
                    //{
                    //    try
                    //    {
            ...
            //double commandElapsedSeconds = 0;// commandExecution.Elapsed.TotalSeconds;
