...
#region ------------- Constants ---------------
        ...
        //static IObjectPool _objectPool;

        //static ConnectionManager()
        //{
        //    _objectPool = ObjectPoolManager.TryGetPool(typeof(Common.Protobuf.InsertCommand));
        //}
        ...
        public ConnectionManager(StatisticsCounter statsCollector)
        {
            ...
            if (threadsPerProcessor > 0)
            {
                ...
                // EventLogUtil.LogEvent("Threads.." + threadsPerProcessor, System.Diagnostics.EventLogEntryType.Error);
            }
            ...
            //ThreadPool.SetMaxThreads(Environment.ProcessorCount * 2 * 2 * 2, Environment.ProcessorCount * 2);
        ...
        private static void AssureSend(ClientManager clientManager, IList buffer, Array userpayLoad, Alachisoft.NCache.Common.Enum.Priority priority)
        {
            try
            {
                ...
                //if (SocketServer.IsServerCounterEnabled)
                //{
                //    clientManager.ConnectionManager.PerfStatsColl.IncrementResponsesQueueSizeStats(buffer.Count);
                //}
            }
        ...
        private static void AssureRecieve(ClientManager clientManager, ref byte[] buffer)
        {
            ...
            do
            {
                try
                {
                    ...
                    //if (bytesRecieved == 0) throw new SocketException();
                }
                catch (SocketException se)
                {
        ...
        private void CallbackThread()
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    {
                        // if (SocketServer.IsServerCounterEnabled) SocketServer.Logger.NCacheLog.Error("ConnectionManager.CallbackThread", e.ToString());
                    }
                }
            }
...
#region Optimized path for Version 4.9 SP1
        ...
        internal void OnReceiveBuffer(object sender, SocketAsyncEventArgs socketEventArgs)
        {
            ...
            try
            {
                do
                {
                    ...
                    //if (SocketServer.IsServerCounterEnabled)
                    //    PerfStatsColl.IncrementBytesReceivedPerSecStats(socketEventArgs.BytesTransferred);

                    CommandBuffer cmdBuffer = receiveContext.UpdateBytesReceived();
                    receiveContext.Deserializer.AddCommandBuffer(cmdBuffer);

                    //ReceiveBufferAsync(receiveContext, !receiveContext.Deserializer.BusyProcessing);
                    //ReceiveBufferAsync(receiveContext, !receiveContext.Deserializer.BusyProcessing);
                   // ReceiveBufferAsync(receiveContext, false);

                    object command;
                    long ackId = -1;
                    short cmdType;
                    bool waitForResponse;
                    //receiveContext.Deserializer.DeserializeCommand(out command, out ackId, out cmdType, this);
                    if (!receiveContext.Deserializer.BusyProcessing)
                    {
                        ...
                        ThreadPool.QueueUserWorkItem(OnProcesRequest,receiveContext);
                            ...
                            //    else
                            //    {
                            //        //ProcessClientCommand(command, cmdType, receiveContext.ClientManager, 0, -1, null);
                            ...
                            //    //if (SocketServer.IsServerCounterEnabled)
                            //    //    PerfStatsColl.IncrementRequestsPerSecStats(1);
                            //    // ProcessClientCommand(command, cmdType, receiveContext.ClientManager, ++_requestId, ackId, null);
                            //    //}
                            //    //else
                            //    //  ProcessClientCommand(command, cmdType, receiveContext.ClientManager, ++_requestId, ackId, null);
                            //    //   ProccessCommand(command, receiveContext.ClientManager);
                            //}
                        
                    }
                } while (!ReceiveBufferAsync(receiveContext, false));

                // x. Confirm the .Net core's stuff -->
            }
            catch (SocketException so_ex)
            {
        ...
        private static void BeginAsyncSendNew(SocketAsyncEventArgs args, byte[] buffer, int offset, int count, ClientManager cmanager = null)
        {
            ...
            //    {
            //        UserToken = new ReceiveStruct() { client = cmanager }
            //    };

            //    SendEventArgs.SetBuffer(buffer, offset, count);
            //    SendEventArgs.Completed += OnCompleteAsyncSend;
            //}
            //if(buffer != null)
            //    Array.Copy(buffer, 0, SendEventArgs.Buffer, 0, buffer.Length);
            SendContextServer sendStruct = (SendContextServer)args.UserToken;

            ClientManager clientManager = sendStruct.Client;

            // ArraySegment<byte> newSegment = new ArraySegment<byte>(buffer, offset, count);
