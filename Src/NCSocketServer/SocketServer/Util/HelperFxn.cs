...
namespace Alachisoft.NCache.SocketServer.Util
{
    internal sealed class HelperFxn
    {
        ...
        public static int ToInt32(byte[] buffer, int offset, int size, string s)
        {
            ...
            {
//                StreamWriter writer = new StreamWriter("c:\\trace.txt");
//                writer.WriteLine("Error occoured while getting " + s + " size");
//                writer.WriteLine("Buffer Length: " + buffer.Length + "\r\nBuffer Data:\r\n");
//                for (int i = 0; (i < 8 || i >= buffer.Length); i++)
//                    writer.WriteLine(buffer[i]);
//                writer.WriteLine("Its string representation: " + UTF8Encoding.UTF8.GetString(buffer));
//                writer.WriteLine("Last command received :" + ConnectionManager.lastCommand + " cmd-length :" + ConnectionManager.lastCmdLengh
//                                 + " data length " + ConnectionManager.lastCmdDataLength);
//                writer.Close();
//                writer.Dispose();
                throw;
            }
