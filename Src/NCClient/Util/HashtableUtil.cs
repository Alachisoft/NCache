...
namespace Alachisoft.NCache.Client
{
    ...
    /// </summary>
    internal static class HashtableUtil
    {
        /// <summary>
        ...
        internal static string ToString(Hashtable table)
        {
            if (table != null)
            {
        ...
        /// <returns></returns>
        internal static string ToString(ArrayList list)
        {
            if (list != null)
            {
                StringBuilder toStr = new StringBuilder();
        ...
        internal static Hashtable FromString(string rep)
        {
        ...
        internal static IDictionary<string, Exception> ToDictionary(Hashtable hashtable)
        {
            IDictionary<string, Exception> keyValuePairs = new Dictionary<string, Exception>();

            foreach (var key in hashtable.Keys)
            {
                keyValuePairs.Add((string)key, (Exception)hashtable[key]);
            }

            return keyValuePairs;
        }

        internal static IDictionary<string, bool> ExtractKeyStatus(Hashtable hashtable, string[] keys)
        {
        ...
        internal static Hashtable CreateCustomComparerHashtable(Hashtable table)
        {
            if (table == default(Hashtable))
                return default(Hashtable);

            var syncedTable = new Hashtable(new Common.DataStructures.EqualityComparer());

            foreach (DictionaryEntry entry in table)
            {
                var key = entry.Key;
                var value = entry.Value;
                var possiblyHashtableValue = value as Hashtable;

                if (possiblyHashtableValue != default(Hashtable))
                    value = CreateCustomComparerHashtable(possiblyHashtableValue);

                syncedTable[key] = value;
            }
            return syncedTable;
        }
    }
