...
namespace Alachisoft.NCache.Client
{
    internal class Request
    {
        ...
        private Dictionary<Common.Net.Address, Dictionary<int, ResponseList>> _responses;// = new Dictionary<Common.Net.Address, Dictionary<int, ResponseList>>();
        ...
        private void DeserializeRawResponsesIfPresent()
        {
            if (_rawResponses != null && _rawResponses.Count >0)
            {
                for (int i = 0; i < _rawResponses.Count; i++)
                {
                    ...
                    //if (!_responses.ContainsKey(_rawResponses[i].SequenceId))
                    //    _responses.Add(_rawResponses[i].SequenceId, _rawResponses[i]);
                }
                _rawResponses.Clear();
            }
        }
