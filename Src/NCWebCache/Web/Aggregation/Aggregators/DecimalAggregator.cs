...
namespace Alachisoft.NCache.Web.Aggregation
{
    ...
    public class DecimalAggregator : IAggregator
    {
        ...
        object result = new Decimal(0.000);
        ...
        private object Calculate(ICollection value, bool calculate)
        {
            switch (aggregateType)
            {
                case AggregateFunctionType.SUM:
                    result = (Decimal)Calculation.Sum(value, DataType.DECIMAL);
                    break;
                case AggregateFunctionType.AVG:
                    result = Calculation.Avg(value, DataType.DECIMAL, calculate);
                    break;
                case AggregateFunctionType.MIN:
                    result = (Decimal)Calculation.Min(value, DataType.DECIMAL);
                    break;
                case AggregateFunctionType.MAX:
                    result = (Decimal)Calculation.Max(value, DataType.DECIMAL);
                    break;
            }

            if (result is decimal)
                result = decimal.Parse(((decimal)result).ToString("N04"));
