...
namespace Alachisoft.NCache.Caching.Topologies.Clustered
{
    ...
    internal class ReplicatedCacheBase : ClusterCacheBase
    {
        ...
        {
            public Identity(bool hasStorage, int renderPort, IPAddress renderAddress, string mapping = null) : base(hasStorage, renderPort, renderAddress, mapping) { }
        ...
        protected CacheAddResult Clustered_Add(ArrayList dests, object key, CacheEntry cacheEntry, string taskId, OperationContext operationContext)
        {
            ...
            try
            {
                ...
                if (cacheEntry.Notifications != null)
                {
            ...
            finally
            {
                ...
                if (entryToBeSent != null)
                    entryToBeSent.MarkFree(NCModulesConstants.Topology);
                ...
                if (!ReferenceEquals(cacheEntry, entryToBeSent))
                    MiscUtil.ReturnEntryToPool(entryToBeSent, Context.TransactionalPoolManager);
            }
            return result;
        }
        ...
        /// or invokes <see cref="Local_Add"/> locally.
        /// </remarks>
        protected bool Clustered_Add(ArrayList dests, object key,  OperationContext operationContext)
        {
            bool result = false;
            try
            {
                /// Ask every server to add the object, except myself.
                Function func = new Function((int)OpCodes.AddSyncDependency, new object[] { key, operationContext }, false, key);
                RspList results = Cluster.BroadcastToMultiple(dests,
                    func,
                    GroupRequest.GET_ALL, _asyncOperation);
        ...
        protected CacheInsResultWithEntry Clustered_Insert(ArrayList dests, object key, CacheEntry cacheEntry, string taskId, object lockId, LockAccessType accessType, OperationContext operationContext)
        {
            ...
            try
            {
                ...
                retVal = ClusterHelper.FindAtomicInsertStatusReplicated(results,_context.TransactionalPoolManager);
            ...
            finally
            {
                ...
                if (entryToBeSent != null && !ReferenceEquals(cacheEntry, entryToBeSent))
                    MiscUtil.ReturnEntryToPool(entryToBeSent, Context.TransactionalPoolManager);
            }
        }
        ...
        /// </remarks>
        protected CacheEntry Clustered_Remove(object key, ItemRemoveReason ir, Caching.Notifications notification, string taskId, string providerName, bool notify, object lockId, ulong version, LockAccessType accessType, OperationContext operationContext)
        {
            CacheEntry retVal = null;
            try
            {
                if (ServerMonitor.MonitorActivity) ServerMonitor.LogClientActivity("RepCacheBase.Remove", "enter");
                Function func = new Function((int)OpCodes.Remove, new object[] { key, ir, notify, notification, taskId, lockId, accessType, version, providerName, operationContext }, false, key);
        ...
        /// This method invokes <see cref="handleRemove"/> on every server node in the cluster.
        /// </remarks>
        protected Hashtable Clustered_Remove(ICollection keys, ItemRemoveReason ir, Caching.Notifications notification, string taskId, string providerName, bool notify, OperationContext operationContext)
        {
        ...
        protected bool Clustered_Remove(ICollection keys, ItemRemoveReason reason, OperationContext operationContext)
        {
            try
            {
                ...
                //Cluster.BroadcastToServers(func, GroupRequest.GET_ALL, true);
                func.Cancellable = true;
        ...
        #region ReplicatedStateTransfer
        ...
        internal ReplicaStateTxfrInfo Clustered_GetEntries(Address targetNode)
        {
            ...
            {
                Context.NCacheLog.Error("ReplicatedCacheBase.Clustered_GetEntries()", e.ToString());
                throw;
            }
            catch (Exception e)
            {
