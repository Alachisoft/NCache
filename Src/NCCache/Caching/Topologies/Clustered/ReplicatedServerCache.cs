    ...
    internal class ReplicatedServerCache : ReplicatedCacheBase, IPresenceAnnouncement, ICacheEventsListener
    {
        ...
        protected override void Initialize(IDictionary cacheClasses, IDictionary properties)
        {
            ...
            try
            {
                ...
                InitializeCluster(properties, Name, MCAST_DOMAIN, new Identity(true, (_context.Render != null ? _context.Render.Port : 0), (_context.Render != null ? _context.Render.IPAddress : null), (_context.Render != null ? _context.Render.ServerMapping : null)));
        ...
        public override void Initialize(IDictionary cacheClasses, IDictionary properties, bool twoPhaseInitialization)
        {
            ...
            try
            {
                ...
                InitializeCluster(properties, Name, MCAST_DOMAIN, new Identity(true, (_context.Render != null ? _context.Render.Port : 0), (_context.Render != null ? _context.Render.IPAddress : null), (_context.Render != null ? _context.Render.ServerMapping : null)), twoPhaseInitialization, false);
                _stats.GroupName = Cluster.ClusterName;
        ...
        protected class StateTransferTask : AsyncProcessor.IAsyncTask
        {
            ...
            private void AddEntryToCache(string key, CacheEntry val, OperationContext operationContext)
            {
                try
                {
                    ...
                    if (_parent.Local_Contains(key, operationContext))
                    {
                        return;
                    }
        ...
        private object handleRemove(Address src, object info)
        {
            try
            {
                ...
                if (info is object[])
                {
                    ...
                    {
                        object tmp = args[0];
                        if (tmp is object[] || tmp is IList)
                        {
                            if (args.Length > 5)
                            {
                                providerName = args[5] as string;
                            }
                            result = Local_Remove(tmp is IList ? (IList)tmp : (object[])tmp, ItemRemoveReason.Removed, src, notification, taskId, providerName, true, operationContext);
        ...
        void ICacheEventsListener.OnCustomRemoveCallback(object key, object value, ItemRemoveReason removalReason, OperationContext operationContext, EventContext eventContext)
        {
            try
            {
                ...
                if (notification != null)
                {
                    ...
                    if (_allowEventRaiseLocally)
                    {
                        NotifyCustomRemoveCallback(key, notification.ItemRemoveCallbackListener, removalReason, true, operationContext, eventContext);
                    }
