...
namespace Alachisoft.NCache.Caching.Topologies.Clustered
{
    public class DistributionManager : IDistributionManager
    {
        ...
        protected Dictionary<Address, Dictionary<int, HashMapBucket>> _POTeamBucketsOwnershipMap;
        protected Hashtable _bucketsStats;
        protected ArrayList _existingMembers;
        protected ArrayList _permanentMembers;
        ...
        protected Hashtable _orphanedBuckets;
        protected Address _leavingNode =null;
        private ILogger _ncacheLog;
        public int mapVersion = 0;
        List<int> localBucketIds = new List<int>();

        List<int> permenantlocalBucketIds = new List<int>();
        private bool _isValidateBucket= true;
        public int MapVersion { get; set; } = 0;
        ...
        public DistributionManager(int autoBalancingThreshold, long cacheSizePerNode, DistributionMaps distributionMaps, ArrayList existingMembers, Hashtable bucketStats, ILogger log, int totalBuckets = -1, bool isValidateBucket= true)
        {
            TotalBuckets = (totalBuckets>=50 && totalBuckets<=1000)? totalBuckets : (distributionMaps.Hashmap != null ? distributionMaps.Hashmap.Count : 1000);
            Initialize();
            _autoBalancingThreshold = autoBalancingThreshold;
            _cacheSizePerNode = cacheSizePerNode;
            NCacheLog = log;
            _bucketsStats = bucketStats;
            _existingMembers = existingMembers;
            _isValidateBucket = isValidateBucket;

            if (distributionMaps!= null)
            {
                _lastCreatedHashMap = distributionMaps.Hashmap;
                _installedHashMap = distributionMaps.Hashmap;
                _bucketsOwnershipMap = distributionMaps.BucketsOwnershipMap; 
            }
        }
        ...
        private void Initialize()
        {
            ...
            _bucketSize = (int)Math.Ceiling((double)(intRange * -1) / (double)TotalBuckets);
        }
        /// <summary>
        ...
        public ArrayList Members
        {
            ...
            set { _existingMembers = value;  }
        }

        public ArrayList PermanentMembers
        {
            get { 
                if(_permanentMembers== null )
                {
                    GetPermanentAddress();
                }
                return _permanentMembers; }
        }
        ...
        public ArrayList CandidateNodesForBalance
        {
            get
            {
                ...
                candidateNodes.Sort();
                if (NCacheLog!=null && NCacheLog.IsInfoEnabled) NCacheLog.Info("DistributionMgr.CandidateNodesForBalance", sb.ToString());
        ...
        public Hashtable BucketsOwnershipMap
        {
            ...
            set { _bucketsOwnershipMap = value; _POTeamBucketsOwnershipMap = POTeamConvertOwnershitMap(_bucketsOwnershipMap); }
        }

        public List<int> LocalBucketIds
        {
            get
            { 
                return localBucketIds;
            }
        }

        public List<int> PermanentLocalBucketIds
        {
            get
            {
                return permenantlocalBucketIds;
            }
        }
        ...
        public bool IsBucketFunctional(Address owner, object key)
        {
            ...
            return IsBucketFunctional(owner, bucketId);
        }

        public bool IsBucketFunctional(Address owner, int bucketId)
        {
            Sync.AcquireReaderLock(Timeout.Infinite);
            try
            {
                if (_POTeamBucketsOwnershipMap != null && _POTeamBucketsOwnershipMap.ContainsKey(owner))
                {
                    var buckets = _POTeamBucketsOwnershipMap[owner];
                    if (buckets != null)
                    {
                        HashMapBucket bucket;
                        buckets.TryGetValue(bucketId, out bucket);
                        if (bucket != null && bucket.Status == BucketStatus.Functional)
                        {
                            return true;
                        }
                    }
            ...
            try
            {
                if (_installedHashMap != null)
                {
                    ...
                    if (bucket != null)
                    {
                        ...
                        Address coordinatorNodeAddress = bucket.TempAddress; // this should be the sub-coordinator address.
                        Dictionary<int, HashMapBucket> ownershipMap;
                        _POTeamBucketsOwnershipMap.TryGetValue(coordinatorNodeAddress, out ownershipMap);

                        if (ownershipMap == null)
                        {
                            if(NCacheLog != null) NCacheLog.Warn("DistributionManager.SelectNodeInternal()", "ownershipMap is null. Returning permanent address of bucket.");
                            return bucket.PermanentAddress;
                        }

                        HashMapBucket ownedBucket;
                        ownershipMap.TryGetValue(bucket.BucketId, out ownedBucket);
                        if (ownedBucket != null)
                        {
        ...
        public virtual bool IsMyBackUpNodeUunderMaintaince(Address server)
        {
            return false;
        }
        /// <summary>
            ...
            try
            {
                ...
                switch (distInfoData.ClustActivity)
                {
                    case ClusterActivity.NodeJoin:
                        ...
                        {
                            if (NCacheLog != null && NCacheLog.IsErrorEnabled) NCacheLog.Error("DistributionMgr.GetMaps()", e.ToString());
                            break;
                        }
                    ...
                    case ClusterActivity.None:
                        ...
                        DistributionMaps result = bnMgr.BalanceNodes(distInfoData, _lastCreatedHashMap, _bucketsStats, _existingMembers, TotalBuckets);
        ...
        public virtual bool IsPreviousNode()
        {
            return false;
        }

        public virtual bool HasUndermaintenceReplica()
        {
            return false;
        }

        public virtual Address GetUndermaintenceReplica(Address server)
        {
            return null;
        }
        ...
        {
            if (NCacheLog != null && NCacheLog.IsInfoEnabled) NCacheLog.Info("DistributionMgr.GetMapsOnNodeJoining()", "Total Data Size : " + TotalDataSize.ToString());
            ArrayList tmpMap = null;
            Hashtable bucketsOwnershipMap = null;
            ArrayList partitionNodes = new ArrayList();

            ArrayList newHashMap = DistributeHashMap.BalanceBuckets(distInfoData, _lastCreatedHashMap, _bucketsStats, _existingMembers, _cacheSizePerNode, TotalBuckets, NCacheLog);
            _existingMembers.Add(distInfoData.AffectedNode.NodeAddress);
            ...
            bucketsOwnershipMap = GetBucketsOwnershipMap(_lastCreatedHashMap);
            if (NCacheLog != null && NCacheLog.IsInfoEnabled) NCacheLog.Info("DistributionMgr.GetMaps()", "Sending new map as a new node joined the cluster");
            return new DistributionMaps(_lastCreatedHashMap, bucketsOwnershipMap);
        }

        protected DistributionMaps GetMapsOnNodeReJoining(DistributionInfoData distInfoData)
        {
            if (NCacheLog != null && NCacheLog.IsInfoEnabled) NCacheLog.Info("DistributionMgr.GetMapsOnNodeJoining()", "Total Data Size : " + TotalDataSize.ToString());
            ...
            _existingMembers.Add(distInfoData.AffectedNode.NodeAddress);
            Address replicaAddress = GetReplica(distInfoData.AffectedNode);

            ArrayList newHashMap = new ArrayList();

            foreach (HashMapBucket bucket in _lastCreatedHashMap)
            {
                foreach (HashMapBucket orphanBucket in _orphanedBuckets.Values)
                {
                    if (bucket.BucketId.Equals(orphanBucket.BucketId))
                    {
                        bucket.TempAddress = orphanBucket.PermanentAddress;
        ...
        protected virtual Address GetReplica(PartNodeInfo node)
        {
            return null;
        }
        ...
        protected DistributionMaps GetMapsOnNodeLeaving(DistributionInfoData distInfoData)
        {
            ...
            if (NCacheLog != null && NCacheLog.IsInfoEnabled) NCacheLog.Info("DistributionMgr.GetMapsOnNodeLeaving()", "Before Calling DistributeOrphanBuckets() ---- Leaving Node:" + distInfoData.AffectedNode.NodeAddress.ToString() + " Existing Members Count:0" + _existingMembers.Count);
            Hashtable orphanedBuckets = null;
            tmpMap = DistributeHashMap.DistributeOrphanBuckets(_lastCreatedHashMap, distInfoData.AffectedNode.NodeAddress, _existingMembers, out orphanedBuckets);
            ...
            DistributionMaps distributionMaps = new DistributionMaps(_lastCreatedHashMap, bucketsOwnershipMap);
            if (distInfoData.IsUnderMaintenance)
            {
                _orphanedBuckets = orphanedBuckets;

                distributionMaps.OrphanedBuckets = _orphanedBuckets;
            }
        ...
        internal Hashtable GetOwnerHashMapTable(Hashtable renderers)
        {
            ...
            try
            {
                ...
                if (_installedHashMap != null)
                {
                    ...
                    if (_orphanedBuckets != null && _orphanedBuckets.Count > 0)
                    {
                        foreach (ArrayList bucketList in _bucketsOwnershipMap.Values)
                        {
                            foreach (HashMapBucket bucket in bucketList)
                            {
                                if (_orphanedBuckets.ContainsKey(bucket.BucketId))
                                {
                                    ownerHashCodeTable[bucket.BucketId] = GetServerAddress(renderers, bucket.PermanentAddress);
                                }
                            }
                        }
                    }
        ...
        /// <summary>
        /// Returns a table that has buckets current owners and there respective hashcode ranges
        /// </summary>
        /// <returns>a hashtable, with bucket id's as keys and owners address as value</returns>
        internal Hashtable GetOwnerModuleHashMapTable(Hashtable renderers)
        {
            Hashtable ownerHashCodeTable = new Hashtable(TotalBuckets);
            try
            {
                    ...
                    for (int i = 0; i < _installedHashMap.Count; i++)
                    {
                        ...
                        if (bucket != null)
                        {
                            switch (bucket.Status)
                            {
                                ...
                                case BucketStatus.UnderStateTxfr:
                                case BucketStatus.Functional:
                                    ownerHashCodeTable.Add(i, GetServerAddressForModule(renderers, bucket.PermanentAddress));
                                    break;
                            }
                        }
                    }
                    ...
                    if (_orphanedBuckets != null && _orphanedBuckets.Count > 0)
                    {
                        foreach (ArrayList bucketList in _bucketsOwnershipMap.Values)
                        {
                            foreach (HashMapBucket bucket in bucketList)
                            {
                                if (_orphanedBuckets.ContainsKey(bucket.BucketId))
                                {
                                    ownerHashCodeTable[bucket.BucketId] = GetServerAddressForModule(renderers, bucket.PermanentAddress);
                                }
                            }
                        }
                    }
        ...
        public static string GetServerAddressForModule(Hashtable renderers, Address clusterAddress)
        {
            string serverAddress = string.Empty;
            if (clusterAddress != null && clusterAddress.IpAddress != null) {

                serverAddress = clusterAddress.IpAddress.ToString();
                // if render not null and render contain ip then erverAddress reset 
                // otherwise return clusterAddress.IpAddress
        ...
        /// <summary>
        /// Returns a table that has buckets permanent owners and there respective hashcode ranges
        /// </summary>
        /// <param name="bucketSize">out parameter, holds the individual bucket size</param>
        /// <returns>a hashtable, with bucket id's as keys and owners address as value</returns>        
        public Hashtable GetOwnerModuleHashMapTable(Hashtable renderers,  out int bucketSize)
        {
            bucketSize = this.BucketSize;
            return GetOwnerModuleHashMapTable(renderers);
        }

        public void UpdateBucketStats(BucketStatistics[] localBuckets, Address address)
        {
            try
            {
                ...
                {
                    BucketStatistics[] bucketStats = localBuckets.Clone() as BucketStatistics[];
                    if (bucketStats != null)
                    {
                        ...
                        for (int i = 0; i < bucketStats.Length; i++)
                        {
                            ...
                            if (bucketStats.Length!= TotalBuckets && _installedHashMap.Count <= i)
                            {
                                NCacheLog?.Info($"DistributionMananger.updateBucketStats: i={i}, _installedHashMap.Count= {_installedHashMap.Count} ");
                            }
                            if (bucketStats.Length != TotalBuckets &&_bucketsStats.Count < i)
                            {
                                NCacheLog?.Info($"DistributionMananger.updateBucketStats: i={i}, _bucketsStats.Count= {_bucketsStats.Count} ");
                            }
                            HashMapBucket bucket = (HashMapBucket)_installedHashMap[i];
                            if (bucket.PermanentAddress.Equals(address))
                            {
                                if(bucketStats[i] != null)
                                    _bucketsStats[i] = bucketStats[i];
                            }
                            else
                            {
                            }
                    ...
                    if (NCacheLog != null && NCacheLog.IsInfoEnabled) NCacheLog.Info("DistributionMgr.UpdateBucketStats()", "bucketStats = " + _bucketsStats == null ? "null" : _bucketsStats.Count.ToString());
                }
            }
            catch (Exception e)
            {
                ...
                while (true)
                {
                    ...
                    lock (_status_wait_mutex)
                    {
                        if (!owner.Equals(hashBucket.TempAddress) || hashBucket.StateTxfrLatch.IsAnyBitsSet(status)
                            || (owner.Equals(hashBucket.TempAddress) && hashBucket.TempAddress.Equals(hashBucket.PermanentAddress)))//this condition handles anamoly if bucket status is mistakenly set to needtransfer
                            return;
        ...
        public List<int> GetBucketIdOwnedbyMe()
        {
            List<int> myBuckets = new List<int>();
            _sync.AcquireReaderLock(Timeout.Infinite);
            try
            {
        ...
        public List<int> GetBucketIdPermanentlyOwnedbyMe()
        {
            ...
            try
            {
                ...
                Hashtable bucketsNeedTransfer = new Hashtable();

                if (_localAddrss != null)
                    mybuckets = _bucketsOwnershipMap[_localAddrss] as ArrayList;
                if (mybuckets != null)
                {
                    foreach (HashMapBucket bucket in mybuckets)
                    {
                        if (bucket.PermanentAddress.Equals(_localAddrss))
                        {
                            if (!myBuckets.Contains(bucket.BucketId))
                                myBuckets.Add(bucket.BucketId);
                        }
                        else
                            bucketsNeedTransfer.Add(bucket.BucketId, String.Empty);
                    }
                }
                ...
                while (ide.MoveNext())
                {
                    if (ide.Key == _localAddrss) continue;
                    ...
                    if (buckets != null)
                    {
                        foreach (HashMapBucket bucket in buckets)
                        {
                            if (bucket.PermanentAddress.Equals(_localAddrss) && !bucketsNeedTransfer.Contains(bucket.BucketId))
                            {
                                if (!myBuckets.Contains(bucket.BucketId))
                                    myBuckets.Add(bucket.BucketId);
                            }
        ...
        public IList GetMyFunctionalBucketIds()
        {
            IList myFunctionalBuckets = new List<int>();
            _sync.AcquireReaderLock(Timeout.Infinite);
            try
            {
                ArrayList bucketsList = GetBucketsList(_localAddrss);
                if(bucketsList != null)
                {
                    var myBuckets = bucketsList.Cast<HashMapBucket>();
                    if(myBuckets != null)
                        myFunctionalBuckets = myBuckets.Where(x => x.Status == BucketStatus.Functional).Select(x => x.BucketId).ToList();
                }
            ...
            return myFunctionalBuckets;
        }

        public ArrayList GetOrphanBucketsAssignedToMe()
        {
            _sync.AcquireReaderLock(Timeout.Infinite);
            ArrayList myBuckets = null;
            ArrayList orphanBucketsAssignedToMe = new ArrayList();
            try
            {
                myBuckets = _bucketsOwnershipMap[_localAddrss] as ArrayList;
                if (myBuckets != null)
                {
                    myBuckets = myBuckets.Clone() as ArrayList;

                    foreach (var bucket in myBuckets)
                    {
                        if (_orphanedBuckets.ContainsKey(bucket))
                            orphanBucketsAssignedToMe.Add(bucket);
                    }
                }
            }
            catch (NullReferenceException) { }
            ...
            return orphanBucketsAssignedToMe;
        }
        ...
        public void ChangeBucketStatusToStateTransfer(ArrayList buckets, Address node)
        {
            ...
            try
            {
                if (buckets != null)
                {
                    ...
                    {
                        Dictionary<int, HashMapBucket> nodeBuckets;
                        _POTeamBucketsOwnershipMap.TryGetValue(node, out nodeBuckets);
                        if (nodeBuckets != null)
                        {
                            foreach (int bucketId in buckets)
                            {
                                HashMapBucket bucket;
                                nodeBuckets.TryGetValue(bucketId, out bucket);
                                if (bucket != null && node.Equals(bucket.TempAddress))
                                {
                                    bucket.Status = BucketStatus.UnderStateTxfr;
                                }
                            }
        ...
        public virtual Hashtable LockBuckets(ArrayList buckets, Address requestingNode)
        {
                ...
                if (buckets != null)
                {
                    ...
                    while (ie.MoveNext())
                    {
                        lock (InstalledHashMap.SyncRoot)
                        {
                            ...
                            {
                                if (NCacheLog != null && NCacheLog.IsInfoEnabled) NCacheLog.Info("DistributionMgr.lockbuckets", "acquired locked on bucket [" + bucket.BucketId + "] by " + requestingNode);
                            ...
                            {
                                if (NCacheLog != null && NCacheLog.IsInfoEnabled) NCacheLog.Info("DistributionMgr.lockbuckets", "bucket [" + bucket.BucketId + "] owner ship is changed; new owner is " + bucket.TempAddress);
            ...
            {
                if (NCacheLog != null) NCacheLog.Error("DistributionMgr.lockbuckets", e.ToString());
                return result;
            }
            finally
            {
        ...
        public void ReleaseBuckets(ArrayList buckets, Address requestingNode)
        {
            try
            {
                ...
                try
                {
                    ...
                    {
                        Dictionary<int, HashMapBucket> nodeBuckets;
                        _POTeamBucketsOwnershipMap.TryGetValue(requestingNode, out nodeBuckets);
                        ...
                        if (nodeBuckets != null)
                        {
                            foreach (int bucketId in buckets)
                            {
                                HashMapBucket bucket;
                                nodeBuckets.TryGetValue(bucketId, out bucket);
                                if (bucket != null  && requestingNode.Equals(bucket.TempAddress))
                                {
                                    bucket.Status = BucketStatus.Functional;
                    ...
                    UpdateLocalBucketIds();
                    UpdatePermenantLocalBucketIds();
                }
                finally
                {
                    Sync.ReleaseWriterLock();
            ...
            {
                if (NCacheLog != null) NCacheLog.Error("DistributionMgr.ReleaseBuckets", e.ToString());
            }
        }

        public void ReleaseBucket(int bucketId, Address requestingNode)
        {
            try
            {
                Sync.AcquireWriterLock(Timeout.Infinite);
            ...
            {
                NCacheLog.Error("DistributionMgr.ReleaseBucket", e.ToString());
            }
        }
        ...
        public virtual bool VerifyTemporaryOwnership(int bucketId, Address temOwner)
        {
            ...
            {
                if (NCacheLog != null) NCacheLog.Error("DistributionMgr.VerifyTemporaryOwnership", e.ToString());
            }
            catch (Exception e)
            {
                if (NCacheLog != null) NCacheLog.Error("DistributionMgr.VerifyTemporaryOwnership", e.ToString());
        ...
        public bool VerifyPermanentOwnership(int bucketId, Address perOwner)
        {
            ...
            {
                if (NCacheLog != null) NCacheLog.Error("DistributionMgr.VerifyPermanentOwnership", e.ToString());
            }
            return false;
        }
        ...
        public void InstallHashMap(DistributionMaps distributionMaps, ArrayList leftMbrs)
        {
            ...
            try
            {
                ...
                _leavingNode = distributionMaps.LeavingNode;
                _orphanedBuckets = distributionMaps.OrphanedBuckets;
                if (_orphanedBuckets == null) ExitMaintenceMode();
                
                _POTeamBucketsOwnershipMap = POTeamConvertOwnershitMap(newBucketsOwnershipMap);
                ...
                if (_installedHashMap != null)
                {
                    for (int i = 0; i < newMap.Count; i++)
                    {
                        ...
                        if (_isValidateBucket && !oldBucket.PermanentAddress.Equals(newBucket.PermanentAddress) && oldBucket.TempAddress.Equals(newBucket.TempAddress))
                        {
                            if (NCacheLog != null) NCacheLog.Error("Install Hashmap", "BucketID: " + index.ToString() + "\toldBucket: " + oldBucket.PermanentAddress.ToString() + "\toldBucket.Temp: " + oldBucket.TempAddress.ToString() + "\tnewBucket: " + newBucket.PermanentAddress.ToString() + "\tnewBucekt.Temp: " + newBucket.TempAddress.ToString());
                        }
                        else
                        {
                ...
                {
                    NCacheLog?.Info($"DistributionMananger.InstallHashMap: newMap.Count={newMap.Count},_installedHashMap.Count= {_installedHashMap?.Count} ");
            ...
            LogMybuckets(_bucketsOwnershipMap);
        }

        private void UpdateLocalBucketIds()
        {
            lock (localBucketIds)
            {
                localBucketIds = GetBucketIdOwnedbyMe();
                
            }
        }

        private void UpdatePermenantLocalBucketIds()
        {
            lock (permenantlocalBucketIds)
            {
                permenantlocalBucketIds = GetBucketIdPermanentlyOwnedbyMe();

            }
        }


        protected Dictionary<Address, Dictionary<int, HashMapBucket>> POTeamConvertOwnershitMap(Hashtable ownershipMap)
        {
            try
            {
                Dictionary<Address, Dictionary<int, HashMapBucket>> newMap =
                    new Dictionary<Address, Dictionary<int, HashMapBucket>>();
                foreach (DictionaryEntry entry in ownershipMap)
                {
                    ArrayList buckets = entry.Value as ArrayList;
                    Dictionary<int, HashMapBucket> newBuckets = new Dictionary<int, HashMapBucket>();
                    foreach (HashMapBucket bucket in buckets)
                    {
                        newBuckets.Add(bucket.BucketId, bucket);
                    }
                    newMap.Add(entry.Key as Address, newBuckets);
                }
                return newMap;
            }
            catch (Exception e)
            { this._ncacheLog.CriticalInfo("POTEAM", e.ToString()); }
            return null;
        }
        ...
        public void LogMybuckets()
        {
            ...
            if (_bucketsOwnershipMap != null)
            {
                ...
                ArrayList myMap = _bucketsOwnershipMap[_localAddrss] as ArrayList;
        ...
        public virtual void Wait(object key, string group)
        {
            if (key != null)
            {
                if (_installedHashMap != null)
                {
                    ...
                    if (NCacheLog != null) NCacheLog.Error("DistributionManager.Wait", "_installedHashMap == null");
            }
        }
        ...
        internal ArrayList GetPermanentAddress(ArrayList activePartitions)
        {
            ...
            try
            {
                ...
                {
                    if (_bucketsOwnershipMap.ContainsKey(server))
                    {
                        Dictionary<int, HashMapBucket> buckets;
                        _POTeamBucketsOwnershipMap.TryGetValue(server, out buckets);
                        if (buckets != null)
                        {
                            foreach (HashMapBucket bucket in buckets.Values)
                            {
        ...
        internal bool InStateTransfer()
        {
            ...
            try
            {
                foreach (HashMapBucket bucket in _installedHashMap)
                {
                    ...
                    {
                        if(!bucket.PermanentAddress.Equals(bucket.TempAddress)) //Due to a hashmap anomoly, bucket status is mistakenly set to state transfer
                            return true;
                    }
                }
            }
        ...
        internal virtual void ExitMaintenceMode()
        {
        }

        public virtual DistributionInfo GetDistributionInfo()
        {
            return new DistributionInfo()
            {
                DistributionMaps = new DistributionMaps(InstalledHashMap, BucketsOwnershipMap),
                BucketsStats = BucketStats,
                MapVersion = MapVersion                
            };
        }

        public ArrayList GetPermanentAddress()
        {
            _sync.AcquireReaderLock(Timeout.Infinite);
            try
            {
                ArrayList list = new ArrayList();
                foreach (Address server in Members)
                {
                    if (_bucketsOwnershipMap.ContainsKey(server))
                    {
                        Dictionary<int, HashMapBucket> buckets;
        ...
        public int[] GetCountsOfBucketsRequiringStateTransfer()
        {
            int bucketsToSend = 0, bucketsToReceive = 0;
            _sync.AcquireReaderLock(Timeout.Infinite);
            ...
            try
            {
                if (_installedHashMap != null)
                {
                    foreach (var item in _installedHashMap)
                    {
                        HashMapBucket bucket = item as HashMapBucket;
                        
                        if (bucket.Status == BucketStatus.NeedTransfer)
                        {
                            if (bucket.PermanentAddress.Equals(_localAddrss) && !bucket.TempAddress.Equals(_localAddrss)) 
                                bucketsToSend++;
                            else if (!bucket.PermanentAddress.Equals(_localAddrss) && bucket.TempAddress.Equals(_localAddrss)) 
                                bucketsToReceive++;
                        }
                    }
                }
            ...
            return new int[] { bucketsToSend, bucketsToReceive };
        }

        public bool IsBucketUnderStateTransfer(Address owner, int bucketId)
        {
            Sync.AcquireReaderLock(Timeout.Infinite);
            try
            {
                if (_POTeamBucketsOwnershipMap != null && _POTeamBucketsOwnershipMap.ContainsKey(owner))
                {
                    ...
                    if (buckets != null)
                    {
                        HashMapBucket bucket;
                        buckets.TryGetValue(bucketId, out bucket);
                        if (bucket != null && bucket.Status == BucketStatus.UnderStateTxfr)
                        {
                            return true;
                        }
                    }
