...
{
    [Serializable]
    public class Sql7CacheDependency : DBCacheDependency
    {
        public Sql7CacheDependency()
        {         
            base._hintType = ExpirationHintType.Sql7CacheDependency;
        }
        ...
        internal override bool Reset(CacheRuntimeContext context)
        {
            ...
            if (_connectionString != null)
            {
                ...
                {
                    context.ExpiryMgr.CacheDbSyncManager.AddConnectionString(_connectionString, true);
                    context.CacheImpl.ReplicateConnectionString(_connectionString, true);
                }
                if(context.IsDbSyncCoordinator)
        ...
        /// <param name="context"></param>
        /// <returns></returns>
        internal bool Initialize(CacheRuntimeContext context)
        {
            try
            {
                _cdbSyncManager = context.ExpiryMgr.CacheDbSyncManager;
                _cacheName = _cdbSyncManager.GetCacheID(context.CacheImpl.Name);
                _connection = _cdbSyncManager.AddToSqlConnectionPool(_connectionString);
        ...
        {
            return "SQL7DEPENDENCY \"" + _connectionString + "\"" + _dbCacheKey + "\"\r\n";
        }
    }
}