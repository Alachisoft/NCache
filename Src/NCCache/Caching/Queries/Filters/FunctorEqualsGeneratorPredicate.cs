    ...
    public class FunctorEqualsGeneratorPredicate : Predicate, IComparable
    {
        ...
        internal override void ExecuteInternal(QueryContext queryContext, ref SortedList list)
        {
            ...
            if (store != null)
            {
                ...
                if (Inverse)
                    keyList = store.GetData(generator.Evaluate(((MemberFunction)functor).MemberName, queryContext.AttributeValues), ComparisonType.NOT_EQUALS);
                else
                    keyList = store.GetData(generator.Evaluate(((MemberFunction)functor).MemberName, queryContext.AttributeValues), ComparisonType.EQUALS);
