...
namespace Alachisoft.NCache.Common.DataStructures
{
    public class RedBlack<T> : ISizableIndex where T:IComparable
    {
        ...
        private IQueryResult GetWrappedResult(IQueryResult resultKeys, object key,bool checkExclusion)
        {
            int result;
            bool isStringValue = false;
            ...
            {
                resultKeys = new WrapperQueryResult(resultKeys);
                if (checkExclusion)
                {   
                    IDictionaryEnumerator en = GetEnumerator();
                    while (en.MoveNext())
                    {
                        if (isStringValue && en.Key is string)
                            result = en.Key.ToString().ToLower().CompareTo(key.ToString().ToLower());
                        ...
                        {
                            HashVector tmp = en.Value as HashVector;
                            ((WrapperQueryResult)resultKeys).AddExclusion(tmp);
                            break;
                        }    
                    }
                }
