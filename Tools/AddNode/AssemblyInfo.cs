...
[assembly: AssemblyTitle("addnode")]
[assembly: AssemblyDescription("AddNode Utility for NCache")]
...
namespace Alachisoft.NCache.Tools.AddNode
{
    ...
    internal sealed class AssemblyUsage
    {
        ...
        public static void PrintLogo(bool printlogo)
        {
            ...
            string logo = @"Alachisoft (R) NCache Utility AddNode. Version " + Common.ProductVersion.GetVersion() +
                @"
Copyright (C) Alachisoft 2017. All rights reserved.";
        ...
        static public void PrintUsage()
        {
            ...
            string usage = @"Usage: addnode [option[...]].

 cache-id
    Specifies name of clustered cache to which new server node is to be added. 
    Cache must exist on source server.
  
 /x /existing 
    Specifies a server name where the NCache service is running and a cache
    with the specified cache-name is registered. Cache configuration is copied
    from this server to the destination server. 

 /N /new-server
    Specifies a server name where a cache with the specified cache-name needs
    to be registered. The cache configuration is copied from the source server
