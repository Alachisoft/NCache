                ...
                foreach (string server in servers)
                {
                    ...
                    if (m != null)
                    {
                        foreach (string cache in CachesList)
                        {
                            ...
                            {
                                OutputProvider.WriteErrorLine("Failed to stop '{0}' on server {1}. Error: {2} ", cache, getBindIp, e.Message);
                            ...
                            {
                                OutputProvider.WriteErrorLine("Failed to stop '{0}' on server {1}.", cache,getBindIp,e.Message);
