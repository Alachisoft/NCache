...
{
    [Cmdlet(VerbsCommon.Add, "LucenePattern")]
    public class AddLucenePatternBase : AddLucenePatternParameters, IConfiguration
    {
        private string TOOLNAME = "AddLucenePattern Tool";
        ...
        private LuceneDeployment[] GetPatterns(Hashtable patterns)
        {
            LuceneDeployment[] deployments = new LuceneDeployment[patterns.Count];
            IDictionaryEnumerator enu = patterns.GetEnumerator();
            int index = 0;
            while (enu.MoveNext())
            {
